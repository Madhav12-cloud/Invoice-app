...
export async function getMyBusinesProfile(req, res){
    try{
        const { userId } = getAuth(req);
...
//to delete business profile
export async function deleteBusinesProfile(req, res){
    try{
        const { userId } = getAuth(req);
        ...
        const profile = await BusinessProfile.deleteOne({ owner:userId }).lean();
        if(!profile){
