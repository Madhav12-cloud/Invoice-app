...
import { getAuth } from "@clerk/express";//test purpose
...
//console.log("Received invoice data:", req.body); //for displaying result on terminal

  //const userId = "test-user"; for without authentication test purpose
  try {
    ...
    //  LIMIT CHECK
    if (userPlan.plan === "starter" && userPlan.invoiceCount >= 5) {
...
export async function getInvoices(req,res){
   ...
   // console.log("AUTH RESULT:", getAuth(req));
    try {
    const {userId} = getAuth(req) || {};
    if(!userId) {
