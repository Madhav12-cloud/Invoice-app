...
} //it will render the image coming from the server 
...
}// this function create unique id
...
//compute the total for subtotal,tax and total
...
function dataURLtoFile(dataurl, filename = "file") { //you can delete function
...
export default function CreateInvoice() {
  ...
  try {
    let token = await getToken().catch(() => null); // ✅ no template
    if (!token) {
...
useEffect(() => {
  ...
  const fetchCount = async () => {
    try {
      const token = await obtainToken(); // ← use stable obtainToken
      if (!token || cancelled) return;

      const res = await fetch("http://localhost:5000/api/invoice/count", {
  ...
  if (isSignedIn) fetchCount();

  return () => { cancelled = true; }; // ← cleanup prevents stale updates
}, [isSignedIn, obtainToken]); // ← correct deps
// useEffect(() => {
//   const fetchCount = async () => {
//     try {
...
//         "http://localhost:5000/api/invoice/count",
//         {
//           headers: {
...
// }, [isSignedIn]);
  ...
  // we can remove the item using index
...
async function handleSave() {
  ...
  // ✅ Block submission early on the frontend before even hitting the server
  if (plan === "starter" && invoiceCount >= 5) {
  // blocked UI is already showing — this line should never be reached
  // but just in case:
  return;
}
  // if (plan === "starter" && invoiceCount >= 5) {
  //   alert("⚠️ You've reached your 5 invoice limit. Upgrade your plan.");
  //   navigate("/pricing");
  //   return;
  // }
  ...
  try {
    ...
    const token = await obtainToken(); // ✅ uses fixed obtainToken
    ...
    if (res.status === 403) {
    alert("⚠️ You've reached your 5 invoice limit. Upgrade your plan.");
    return; // stay on page — blocked UI handles it
  }
      // if (res.status === 403) {
    //   alert("⚠️ You've reached your 5 invoice limit. Upgrade your plan.");
    //   navigate("/pricing");
    ...
    // ✅ Handle 409 - duplicate invoice number
    if (res.status === 409) {
      alert(json?.message || "Invoice number already exists. Please change it.");
      return;
    }
    ...
    if (isEditing) {
      const idx = all.findIndex(
        (x) => x && (x.id === invoice.id || x._id === invoice.id)
      );
...
//       alert("⚠️ Limit reached. Upgrade your plan.");
...
//           : `${API_BASE}/api/invoice`; //it a post route to create the invoice in the mongoDB
//       const method = isEditing && invoice.id ? "PUT" : "POST";
...
//         // 🚨 HANDLE LIMIT (403)
//         if (res.status === 403) {
//           alert("⚠️ You reached your limit (5 invoices). Upgrade your plan.");
//           navigate("/pricing");
//           return;
//         }
...
//       ) {
//         alert(err.message || "Invoice number already exists.");
//         setLoading(false);
//         return;
//       }
...
//   }
  ...
  //you can preview the invoice in this route
  ...
  //for blocking UI
  // ✅ ADD THIS BLOCK just before your main return (
if (plan === "starter" && invoiceCount >= 5 && !isEditing) {
  return (
    <div style={{ textAlign: "center", padding: "60px 20px" }}>
      ...
      <p style={{ color: "#666", marginBottom: "24px" }}>
        You've used all <strong>5 free invoices</strong> on the Starter plan.
        Upgrade to Professional for unlimited invoices.
      </p>
      <button
        onClick={() => navigate("/pricing", { state: { forceShow: true } })}
        style={{
          background: "#4f46e5",
          color: "white",
          padding: "12px 28px",
      ...
      <div className={createInvoiceStyles.headerContainer}>
        {plan === "starter" && (
          <div style={{
            ...
            <strong>
              You have used {invoiceCount} / 5 invoices
            </strong>

            {invoiceCount >= 4 && (
              <p style={{ color: "red" }}>
                ⚠️ You are near your limit. Upgrade soon!
              </p>
            )}
          </div>
        )}
