...
export default function BusinessProfile() {
  ...
  //to save the business profile in the DB
    ...
    try {
      const token = await getAuthToken();
      if (!token) {
        toast.info("You must be signed in to save your business profile.");
        return;
      }
      ...
      setMeta(merged);//saved in the database
      ...
      toast.success(`Profile ${profileId ? "updated" : "created"} successfully.`);
    } catch (err) {
      console.error("Failed to save profile:", err);
      toast.error(err?.message || "Failed to save profile. See console for details.");
    } finally {
      setSaving(false);
    }
  }
...
async function handleClearProfile(e) {
  e.preventDefault()
  if (
    !confirm(
  ...
  try {
    const token = await getAuthToken();
    if (!token) {
      toast.info("You must be signed in to delete your business profile.");
      return;
    }

    const profileId = meta.profileId;
    if (!profileId) {
      toast.info("No profile found to delete.");
      return;
    }
    ...
    toast.error(err?.message || "Failed to delete profile. See console for details.");
  }
}
    ...
    <div className={businessProfileStyles.pageContainer}>
      <div className={businessProfileStyles.headerContainer}>
        ...
        {!isSignedIn && (
          ...
          >
            You are not signed in - changes cannot be saved. Please in to load
            and save your business profile
          </div>
        )}
      </div>
              ...
              <div>
                ...
                <div className={businessProfileStyles.taxContainer}>
                  ...
                  <p className={businessProfileStyles.taxHelpText}>
                    This tax rate will prefill in new invoice. You can adjust
        ...
        <div className={businessProfileStyles.actionContainer}>
          <div className={businessProfileStyles.actionInnerContainer}>
            <div className={businessProfileStyles.actionButtonGroup}>
              ...
              <button
                type="button"
                onClick={e => handleClearProfile(e)}
...
// //it will fetch image coming from  server side
// function resolveImageUrl(url) {
//   if (!url) return null;
//   const s = String(url).trim();
...
//                 value={meta.email || ""}
//                 onChange={(e) => updateMeta("email",e.target.value)}
//                 placeholder="business@exmaple.com" 
//                 />
//             </div>
...
//                 className={businessProfileStyles.resetButton}
//                 >
//                   <ResetIcon className=" w-4 h-4" />Reset Profile
//               </button>
//             </div>
//           </div>
//         </div>
...
//   //to remove the profile the image save in the uploads will get deleted
//   function handleClearProfile() {
//     if (
//       !confirm(
//         "Clear current profile data? This will remove local changes and previews."
