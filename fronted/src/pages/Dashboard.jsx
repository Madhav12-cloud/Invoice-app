...
/* small helpers to make value to uppercases*/
  ...
  const fetchPlan = async () => {
    if (isSignedIn && userId) {
      try {
        ...
        console.log("USER PLAN (Dashboard):", data.plan);
...
// if (!userPlan) return <div>Loading...</div>;
// if (userPlan === "starter") {
//   navigate("/pricing");
//   return null;
// }
  ...
  return (
    <div className={dashboardStyles.pageContainer}>
      ...
      {/* LOADING ERROR STATE */}
      {loading ? (
        <div className=" p-6">Loding invoices...</div>
      ) : error ? (
        <div className=" p-6">
          ...
          <div className=" flex gap-2">
            ...
            {String(error).toLowerCase().includes("unauthorized") && (
              <button onClick={() => navigate("/login")} className=" px-3 py-1 bg-gray-700
              text-white rounded">
                Sign in
              </button>
            )}
      ...
      <div className={dashboardStyles.mainGrid}>
        <div className={dashboardStyles.sidebarColumn}>
          <div className={dashboardStyles.quickStatsCard}>
            <h3 className={dashboardStyles.quickStatsTitle}>Stats Quick </h3>
          ...
          {/* Quick Actions */}
            ...
            <div className="p-6">
              ...
              <div className={dashboardStyles.quickActionsContainer}>
                ...
                // ✅ REPLACE WITH
                  onClick={() => {
                    if (userPlan === "starter" && storedInvoices.length >= 5) {
                      const confirm = window.confirm(
                        "You've used all 5 free invoices.\n\nClick OK to upgrade your plan."
                      );
                      if (confirm) {
                        navigate("/pricing", { state: { forceShow: true } });
                      }
                    } else {
                ...
                // onClick={() => {
                //     if (userPlan === "starter" && storedInvoices.length >= 5) {
                //       alert("Limit reached. Upgrade your plan.");
                //       navigate("/pricing");
                //     } else {
                //       navigate("/app/create-invoice");
                //     }
...
//  button onClick={() => navigate("/pricing"/*"/app/create-invoice"*/ 
